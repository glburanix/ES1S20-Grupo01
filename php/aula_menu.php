<?php
    ...
    if (isset($_GET['p']) && is_numeric($_GET['p'])) {
        ...
        $q = "SELECT COUNT(cod_aula) FROM aulas WHERE nome_aula like '%$where%' ";
    ...
    $ordem = isset($_GET['ordem']) ? $_GET['ordem'] : 'cod_aula';
    ...
    $q = "SELECT cod_aula, nome_aula, semana, cod_prof,semana
            FROM aulas
            WHERE nome_aula like '%$where%'
            ORDER BY $order_by
            LIMIT $inicio, $exiba";
    ...
    if (mysqli_num_rows($r) > 0) {
        ...
        <thead>
            <tr>
                <th width="10%"><strong>
                    <a href="aula_menu.php?ordem=cod_aula">Código</a></strong></th>
                <th width="25%"><strong>
                    <a href="aula_menu.php?ordem=n">Nome da aula</a></strong></th>
                <th width="25%"><strong>
                    <a href="aula_menu.php?ordem=e">Dia da Semana</a></strong></th>
                <th width="20%"><strong>
                    Ações</strong></th>
            </tr>
        </thead><tbody>';
        while ($row = mysqli_fetch_array($r, MYSQLI_ASSOC)) {
            ...
            <td>' . $row['semana'] . '</td>
            <td class="actions">
                <a href="aula_alt.php?cod_aula=' . $row['cod_aula'] . '" class="btn btn-xs btn-warning">
                    Editar</a>
                <a href="aula_exc.php?cod_aula=' . $row['cod_aula'] . '" class="btn btn-xs btn-danger">
                    Excluir</a>
            </td>
            </tr>';
        }
    ...
    if ($pagina > 1) {
        ...
        if($pagina_correta != 1)
        {
            $pag .= '<li class="page-item"><a class="page-link" href="aula_menu.php?s=' . 
            ($inicio - $exiba) . 
...
<main role="main" class="col-md-9 ml-sm-auto col-lg-10 px-4">
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-5 pb-2 mb-3">
        ...
        <div class="col-md-2">
            <a href="aula_cad.php" class="btn btn-primary h2">Inserir Aula</a>
        </div>
        <div class="col-md-6">
            <div class="input-group h2">
                <input type="text" id="busca" class="form-control" placeholder="Pesquisa de Aulas por nome:">
                             <span class="input-group-btn">
                                <a href="#" class="btn btn-primary" onclick="this.href='aula_menu.php?q='+ document.getElementById('busca').value">
